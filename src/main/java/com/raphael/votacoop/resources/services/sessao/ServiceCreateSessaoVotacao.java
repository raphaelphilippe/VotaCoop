...
public class ServiceCreateSessaoVotacao {
	...
	private boolean checkFormatPrazoSessao(String prazoSessao) {
		...
		if( (! listPrazoSessao[1].matches("[0-9]*") ) | (Integer.parseInt(listPrazoSessao[1]) > 59 | Integer.parseInt(listPrazoSessao[1]) < 00)) {
			throw new ValidationsException("No prazo da Sessão de Votação os minutos precisam conter até dois algarismos entre 0 e 9 e ser um número entre 00 e 59 no seguinte formato HH:mm.");
		}
		
		if(prazoSessao == "00:00") {
			throw new ValidationsException("Não é possível iniciar um sessão com o um prazo de 00:00. Caso um prazo válido não seja informado, a sessão terá a duração de 1 minuto.");
		}
